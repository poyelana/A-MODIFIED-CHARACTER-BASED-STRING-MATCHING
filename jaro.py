...
__all__ = ['get_jaro_distance', '_get_matching_characters']
...
def _score(first, second):
    ...
    return (float(len(m1)) / len(shorter) + float(len(m2)) / len(longer) + float(len(m1) - _transpositions(m1, m2)) / len(m1)) / 3.0
